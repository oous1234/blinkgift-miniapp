...
const SearchDrawer: React.FC<SearchDrawerProps> = ({ isOpen, onClose }) => {
  ...
  // Логика обработки завершения свайпа
  const handleDragEnd = (_: any, info: PanInfo) => {
    // Если потянули вниз больше чем на 100px или сделали быстрый свайп вниз
      ...
      {isOpen && (
        <Box position="fixed" top="0" left="0" right="0" bottom="0" zIndex={2000}>
          ...
          <Box
            ...
            // --- Drag Gesture Props ---
            drag="y" // Разрешаем тянуть по Y
            dragConstraints={{ top: 0 }} // Нельзя тянуть выше точки открытия
            dragElastic={{ top: 0, bottom: 0.2 }} // Сопротивление при тяге вниз
            onDragEnd={handleDragEnd} // Обработчик отпускания
            // --- Styles ---
            position="absolute"
            bottom="0"
            left="0"
            ...
            flexDirection="column"
          >
            {/* Drag Handle (Визуальная полоска) */}
            ...
            {isExpanded && (
              <Box
                ...
                overflowY="auto"
                pb="40px"
                // Чтобы драг работал корректно с внутренним скроллом,
                // можно останавливать всплытие событий драга внутри контента,
                // если нужно, чтобы драг работал ТОЛЬКО за шапку.
                // Но сейчас драг работает на всем окне, если контент не скроллится.
